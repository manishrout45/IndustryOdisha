...
var __publicField = (obj, key, value) => __defNormalProp(obj, typeof key !== "symbol" ? key + "" : key, value);
  ...
  const useCKEditorWatchdogContext = () => React.useContext(ContextWatchdogContext);
  ...
  function normalizeStylesMap(styles) {
    return ckeditor5IntegrationsCommon.mapObjectKeys(styles, (key) => {
      if (key.startsWith("--")) {
        return key;
      }
      return ckeditor5IntegrationsCommon.kebabToCamelCase(key);
    });
  }
  ...
  const EditorElement = React.memo(React.forwardRef(({ definition }, ref) => {
  ...
  class CKEditor extends React.Component {
    ...
    async _initializeEditor() {
      const supports = ckeditor5IntegrationsCommon.getInstalledCKBaseFeatures();
      const {
        editor: Editor,
        disableWatchdog,
        watchdogConfig
    ...
    _getMergedConfig(resetData, config) {
      ...
      if (supports.elementConfigAttachment) {
        mappedConfig = ckeditor5IntegrationsCommon.assignElementToEditorConfig(Editor, this.domContainer.current, mappedConfig);
      }
      if (resetData) {
        mappedConfig = ckeditor5IntegrationsCommon.assignInitialDataToEditorConfig(mappedConfig, this.props.data || "");
      }
      return mappedConfig;
    }
    /**
  ...
  function assertMinimumSupportedVersion() {
    switch (ckeditor5IntegrationsCommon.compareInstalledCKBaseVersion("42.0.0")) {
      case null:
  ...
  const useLifeCycleSemaphoreSyncRef = () => {
    ...
    const pendingWaiters = React.useRef([]);
    const refresh = () => {
  ...
  const ROOT_EDITABLE_OPTIONS_ATTRIBUTE = "$rootEditableOptions";
  const EditorEditable = React.memo(React.forwardRef(({ id, editor, rootName }, ref) => {
    var _a;
    const innerRef = React.useRef(null);
    const root = React.useMemo(() => editor == null ? void 0 : editor.model.document.getRoot(rootName), [editor, rootName]);
    const rootEditableOptions = React.useMemo(() => {
      if (!root) {
        return null;
      }
  ...
  const useMultiRootEditor = (props) => {
    ...
    const _getConfig = useRefSafeCallback(() => {
      let mappedConfig = ckeditor5IntegrationsCommon.assignAttributesPropToMultiRootEditorConfig(attributes, props.config || {});
      mappedConfig = appendAllIntegrationPluginsToConfig(mappedConfig);
      return mappedConfig;
    });
    ...
    const _createEditor = useRefSafeCallback(async (initialData, config) => {
      ...
      const { initialData: mergedInitialData, ...mergedConfig } = ckeditor5IntegrationsCommon.assignInitialDataToMultirootEditorConfig(initialData, config);
      const supports = ckeditor5IntegrationsCommon.getInstalledCKBaseFeatures();
      const editor = await /* istanbul ignore next -- @preserve */
  ...
  exports2.ContextWatchdogContext = ContextWatchdogContext;
  exports2.useCKEditorCloud = useCKEditorCloud;
  exports2.useCKEditorWatchdogContext = useCKEditorWatchdogContext;
